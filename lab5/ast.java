    ...
    public void nameAnalysis(SymTable structSymTab, SymTable globalTab) {
    	for (DeclNode node : myDecls) {
            if (node instanceof VarDeclNode) {
                ...
                node.nameAnalysis(structSymTab);
            }
        }
    }    
...
class VarDeclNode extends DeclNode {
    ...
    public SymInfo nameAnalysis(SymTable structSymTab, SymTable globalTab) {
    	SymInfo Info = null;
    	boolean DelBad = false;
        if(myType.type().isVoidType()){
        	ErrMsg.fatal(myId.lineNum(), myId.charNum(), 
                    "Non-function declared void");
        	DelBad = true;
        }
        else if(myType.type().isStructType()){
        	IdNode StructId = ((StructNode)myType).idNode();
        	Info = globalTab.lookupGlobal(StructId.name());
        	
        	if(Info == null ||!Info.getType().isStructDefType()){
        		ErrMsg.fatal(myId.lineNum(), myId.charNum(), 
        ...
        else if(structSymTab.lookupLocal(myId.name()) != null){
        		ErrMsg.fatal(myId.lineNum(), myId.charNum(), 
        	...
        	if(!DelBad){
        		try {
                    if (myType instanceof StructNode) {
                        Info = new StructInfo(((StructNode)myType).idNode());
                    }
                    else {
...
class FnDeclNode extends DeclNode {
     ...
     */
    public SymInfo nameAnalysis(SymTable symTab) {
        SymInfo Info = symTab.lookupLocal(myId.name());
        if(Info != null){
        	ErrMsg.fatal(myId.lineNum(), myId.charNum(), 
                    "Multiply declared identifier.");
        }else{
        ...
        if(Info != null){
        	((FnInfo)Info).addFormals(typeList);
        }
    ...
    public SymInfo nameAnalysis(SymTable symTab) {
    	SymInfo Info = null;
         if(myType.type().isVoidType()){
        	 ...
        	 Info = symTab.lookupLocal(myId.name());
        	 if(Info == null){
    ...
    public void nameAnalysis(SymTable symTab) {
        ...
        if (myLhs instanceof IdNode) {
            ...
            if (sym == null) { // ID was undeclared
                ...
                SymInfo tempSym = ((StructInfo)sym).getStructType().info();
        ...
        // a link to the Sym for the struct type RHSid was declared to be
            ...
            DotAccessExpNode loc = (DotAccessExpNode)myLhs;
        ...
        if (!badAccess) {
            ...
            if (sym == null) { // not found - RHS is not a valid field name
                ...
                if (sym instanceof StructInfo) {
                    myInfo = ((StructInfo)sym).getStructType().info();
                }
            }
        }
